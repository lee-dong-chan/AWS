...
// import key from "../pem/publickey.pem"; 가져올수 없음
...
const encryptPublicRSA = (text) => {
  ...
  const encryted = crypto.publicEncrypt(publickey, buf);
  return encryted.toString("hex");
};
// console.log(encryptPublicRSA("입력해 봅시다"));

const encryptPrivateRSA = (text) => {
  const buf = Buffer.from(text);
  const encryted = crypto.privateEncrypt(privatekey, buf);
  return encryted.toString("hex");
};
// console.log(encryptPrivateRSA("입력해 봅시다"));
...
// console.log(decryptPublicRSA(encryptPrivateRSA("입력해 봅시다")));
...
// console.log(decryptPrivateRSA(encryptPublicRSA("입력해 봅시다1")));
...
//privatekey로 암호화 ==>privatekey로 복호화 --> 불가

// decryptedPrivate = decryptPrivateRSA(encryptedPrivate);
// console.log("decryptedPrivate : ", decryptedPrivate);

//publickey로 암호화 ==>publickey로 복호화--> 불가
